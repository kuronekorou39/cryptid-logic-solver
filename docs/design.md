...
### Phase 4: ヒント確定・ソルバー機能
- [x] 「自分」プレイヤー設定
- [x] ヒント確定機能（自分のヒントを確定）
- [x] 確定ヒントの自動伝播（他プレイヤーから除外）
- [x] 全ON/OFF機能（プレイヤーのヒントを一括切り替え）
- [x] 解の候補探索（ソルバー）機能
- [x] ソルバー結果の重複排除
- [x] 確定ヒント表示（ソルバー結果上部に固定）
- [x] 一覧/詳細表示切り替え
...
1. **AI推論アシスト**: 最も情報量の多い質問を提案
2. **複数言語対応**: 英語UI
3. **PWA化**: オフライン完全対応
4. **ゲーム履歴エクスポート**: JSON形式で保存/読み込み
